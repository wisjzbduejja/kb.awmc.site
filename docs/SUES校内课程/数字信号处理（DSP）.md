...
**答：利用布莱克曼窗设计的线性相位 FIR 低通滤波器，其单位脉冲响应** $h(n)$ **的表达式为：**$$h(n) = \begin{cases} \frac{\sin[\frac{\pi}{4}(n-10)]}{\pi(n-10)} \left[ 0.42 - 0.5 \cos\left(\frac{\pi n}{10}\right) + 0.08 \cos\left(\frac{\pi n}{5}\right) \right], & 0 \le n \le 20,\ n \neq 10 \\ 0.25, & n = 10 \\ 0, & \text{其他} \end{cases}$$
> 以上内容均由ai生成，答案正确，但过程略显繁琐。实际只用到了 $h_d(n)$ ， $w(n)$ ， 以及 $h(n)$ 的公式（还有一个高等数学的求极限）。甚至只需要记住 $h(n) = h_d(n)w(n)$ 即可。

> 理解公式中每个符号的意义对于解题来说是很有帮助，强烈建议**仔细阅读**课本的第7章第2节开头部分的内容。